...
public class ConditionalModelRules extends BaseRules {
	...
	/**
	 * Generate a base record if there are any base columns, or if there is only one
	 * primary key coulmn (in which case we will not generate a primary key class),
	 * or if there is only one BLOB column (in which case we will not generate a
	...
	public boolean generateBaseRecordClass() {
		return introspectedTable.hasBaseColumns() || introspectedTable.getPrimaryKeyColumns().size() == 1

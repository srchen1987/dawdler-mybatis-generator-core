...
public class KotlinArgRenderer {
	public String render(KotlinArg kotlinArg) {
		return renderAnnotations(kotlinArg) + kotlinArg.getName()
	...
	private String renderAnnotations(KotlinArg kotlinArg) {
		return kotlinArg.getAnnotations().stream().collect(CustomCollectors.joining(" ", "", " ")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	}
}
